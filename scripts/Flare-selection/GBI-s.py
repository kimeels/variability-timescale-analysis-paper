...
def plot_lightcurve(dset, ax):
    ...
    ax.errorbar(dset[std_keys.time], dset[std_keys.flux],
                dset[std_keys.flux_err], ecolor='#C6C6C6',
                fmt='o', markerfacecolor='Grey', markeredgecolor='None')
    ax.set_xlabel(dset[std_keys.time_units], fontsize=12)
    ax.set_ylabel(dset[std_keys.flux_units], fontsize=15)
...
def find_flares(dset, zeropoint, sigma):
    ...
    while start_pos is not None:
        ...
        #Generate a mask leaving only this trigger-->fall interval, to find peak
            ...
            ax.plot(t_all_shifted, np.log(f_allx), 'ko')
            ax.plot(t_dec, np.log(f_decx), marker='o', color= 'Gold', linestyle='None')
            ax.plot(t_rise, np.log(f_risex), marker='o', color= 'Lime', linestyle='None')
            ...
            try:
                ...
                values_rise.append(str(np.float(match)))
            ...
            try:
                ...
                #print "Dec params init", init_dec_pars
                ...
                values_decline.append(str(np.float(match)))
            ...
            ## Set the x axis limit such that it overplots the fit of a single flare on the whole light-curve (excluding artificial noise)
            ## To exclude artificial noise, define cuts at 1/3 and 2/3s of the whole time length of the light-curve.
...
def plot_dataset(d):
    ...
    lightcurve_ax.legend(numpoints=1, loc=2)
    #lightcurve_ax.set_xticks((50400, 50600, 50800, 51000, 51200))
    ...
    for marker_set in flares:
        ...
        marker_list = ('p','^','*','v')
        markerpars = dict(markersize = 10, color='Black')
...
if __name__ == '__main__':
    ...
    data_folder = os.path.expanduser('~/Desktop/code-test/data/smoothed/')
